...
static CFL_BOOL channel_readAll(RGT_SINGLE_CHANNELP channel, CFL_BUFFERP buffer, CFL_UINT32 timeout) {
   ...
   if (!channel_isOpen((RGT_CHANNELP)channel)) {
      RGT_LOG_DEBUG(("rgt_channel_unidirectional.channel_readAll(): connection is closed"));
      return CFL_FALSE;
   }
   ...
   if (timeout > 0) {
      ...
      if (!channel_isOpen((RGT_CHANNELP)channel)) {
         RGT_LOG_DEBUG(("rgt_channel_unidirectional.channel_readAll(): error waiting data. connection is closed"));
         channel->isActive = CFL_FALSE;
         return CFL_FALSE;
   ...
   if (packetLen > 0) {
      ...
      if (!channel_isOpen((RGT_CHANNELP)channel)) {
         RGT_LOG_DEBUG(("rgt_channel_unidirectional.channel_readAll(): error reading body. channel is closed"));
         channel->isActive = CFL_FALSE;
         return CFL_FALSE;
      } else if (retVal == CFL_SOCKET_ERROR) {
         rgt_error_set(channel->channel.connectionType, RGT_ERROR_SOCKET, "Error reading data from socket: %ld",
