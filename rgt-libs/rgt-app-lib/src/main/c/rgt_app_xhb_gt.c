/*
 * $Id: gtwin.c,v 1.3.2.1 2005/08/02 12:47:11 fabio Exp $
 */
...
static CFL_BOOL s_terminalMode = CFL_FALSE;
...
static void HB_GT_FUNC(gt_xScreenUpdate(void)) {
   ...
   if (s_pCharInfoScreen != NULL) {
      if (s_uiDispCount == 0 && s_usUpdtTop <= s_usUpdtBottom) {
         ...
         if (s_terminalMode) {
            ...
            if (rgt_app_conn_isActive(conn) && !rgt_error_hasError()) {
               for (iRow = s_usUpdtTop; iRow <= s_usUpdtBottom; iRow++) {
                  for (iCol = s_usUpdtLeft; iCol <= s_usUpdtRight; iCol++) {
                     index = iRow * _GetScreenWidth() + iCol;
                     rgt_app_conn_putChar(conn, iRow, iCol, (CFL_UINT16)s_pCharInfoScreen[index].Char.AsciiChar,
                                          (CFL_UINT8)s_pCharInfoScreen[index].Attributes, 0);
                  }
               }
      ...
      if (s_usOldCurStyle != s_usCursorStyle && (s_uiDispCount == 0 || s_usCursorStyle == SC_NONE)) {
         HB_GT_FUNC(gt_xSetCursorStyle());
      }

      if (s_usCursorStyle != SC_NONE && s_uiDispCount == 0 &&
          (s_csbi.dwCursorPosition.Y != s_sCurRow || s_csbi.dwCursorPosition.X != s_sCurCol)) {
         HB_GT_FUNC(gt_xSetCursorPos());
      }

      if (s_terminalMode && rgt_error_hasError()) {
         rgt_app_handleLastError("GT_SCREENUPDATE");
      }
   }
}
...
static void HB_GT_FUNC(gt_xInitScreenParam(void)) {
   ...
   if (s_pCharInfoScreen != NULL)
      RGT_HB_FREE(s_pCharInfoScreen);

   if (GetConsoleScreenBufferInfo(s_HOutput, &s_csbi))
      s_pCharInfoScreen = (CHAR_INFO *)RGT_HB_ALLOC(_GetScreenWidth() * _GetScreenHeight() * sizeof(CHAR_INFO));
...
void HB_GT_FUNC(gt_SetPos(SHORT sRow, SHORT sCol, SHORT sMethod)) {
   ...
   if (s_terminalMode) {
      if (rgt_app_isConnected() && !rgt_error_hasError()) {
         rgt_app_conn_setCursorPos(rgt_app_getConnection(), sRow, sCol);
      } else if (!rgt_app_inTransactionMode()) {
         hb_vmRequestQuit();
      }
   }
...
void HB_GT_FUNC(gt_SetCursorStyle(USHORT usStyle)) {
   ...
   case SC_SPECIAL2:
      ...
      if (s_terminalMode) {
         if (rgt_app_isConnected() && !rgt_error_hasError()) {
            rgt_app_conn_setCursorStyle(rgt_app_getConnection(), usStyle);
         } else if (!rgt_app_inTransactionMode()) {
            hb_vmRequestQuit();
         }
      }
...
void HB_GT_FUNC(gt_Scroll(USHORT usTop, USHORT usLeft, USHORT usBottom, USHORT usRight, BYTE byAttr, SHORT iRows, SHORT iCols)) {
   ...
   HB_TRACE(HB_TR_DEBUG,
            ("hb_gt_Scroll(%hu, %hu, %hu, %hu, %d, %hd, %hd)", usTop, usLeft, usBottom, usRight, (int)byAttr, iRows, iCols));

   if (hb_gtRectSize(usTop, usLeft, usBottom, usRight, &uiSize) == 0) {
      BYTE *fpBlank = (BYTE *)RGT_HB_ALLOC(iLength);
      BYTE *fpBuff = (BYTE *)RGT_HB_ALLOC(iLength * 2);
      ...
      for (iCount = (iRows >= 0 ? usTop : usBottom); (iRows >= 0 ? iCount <= usBottom : iCount >= usTop);
...
static void HB_GT_FUNC(gt_xPutch(USHORT usRow, USHORT usCol, BYTE byAttr, BYTE byChar)) {
   ...
   if (s_pCharInfoScreen != NULL && usRow < _GetScreenHeight() && usCol < _GetScreenWidth()) {
...
USHORT HB_GT_FUNC(gt_Box(SHORT Top, SHORT Left, SHORT Bottom, SHORT Right, BYTE *szBox, BYTE byAttr)) {
   ...
   SHORT Height;
   SHORT Width;
   USHORT sWidth = _GetScreenWidth(), sHeight = _GetScreenHeight();

   if ((Left >= 0 && Left < sWidth) || (Right >= 0 && Right < sWidth) || (Top >= 0 && Top < sHeight) ||
...
int HB_GT_FUNC(gt_ReadKey(HB_inkey_enum eventmask)) {
   ...
   if (s_terminalMode) {
      if (rgt_app_isConnected() && !rgt_error_hasError()) {
         ch = rgt_app_conn_getKey(rgt_app_getConnection());
         if (rgt_error_hasError()) {
               ...
               if (altisdown) {
                  if (s_irInBuf[s_cNumIndex].Event.KeyEvent.dwControlKeyState & ENHANCED_KEY) {
                     ...
                     if (s_irInBuf[s_cNumIndex].Event.KeyEvent.bKeyDown) {
                        ...
                        case 0x38:
                           /* Alt key ... */
                           if ((s_irInBuf[s_cNumIndex].Event.KeyEvent.dwControlKeyState & 0x04000000))
   ...
   if (s_wRepeated > 0 || s_cNumRead > s_cNumIndex) {
      if (s_irInBuf[s_cNumIndex].EventType == KEY_EVENT) {
      ...
      } else if (b_MouseEnable && eventmask & ~(INKEY_KEYBOARD | INKEY_RAW) && s_irInBuf[s_cNumIndex].EventType == MOUSE_EVENT) {
         ...
         if (eventmask & INKEY_MOVE && s_irInBuf[s_cNumIndex].Event.MouseEvent.dwEventFlags == MOUSE_MOVED) {
            ch = K_MOUSEMOVE;
                    ...
                    s_irInBuf[s_cNumIndex].Event.MouseEvent.dwButtonState & FROM_LEFT_1ST_BUTTON_PRESSED) {
         ...
         } else if (eventmask & INKEY_RDOWN && s_irInBuf[s_cNumIndex].Event.MouseEvent.dwButtonState & RIGHTMOST_BUTTON_PRESSED) {
            if (s_irInBuf[s_cNumIndex].Event.MouseEvent.dwEventFlags == DOUBLE_CLICK) {
               ch = K_RDBLCLK;
            } else {
               ch = K_RBUTTONDOWN;
...
#if defined(HB_ARCH_32BIT) && (defined(__BORLANDC__) || defined(_MSC_VER) || defined(__WATCOMC__) || defined(__MINGW32__))
...
void HB_GT_FUNC(gt_SetDispCP(char *pszTermCDP, char *pszHostCDP, BOOL bBox)) {
   ...
   if (pszTermCDP && pszHostCDP && *pszTermCDP && *pszHostCDP) {
      PHB_CODEPAGE cdpTerm = hb_cdpFind(pszTermCDP), cdpHost = hb_cdpFind(pszHostCDP);
      if (cdpTerm && cdpHost && cdpTerm != cdpHost && cdpTerm->nChars && cdpTerm->nChars == cdpHost->nChars) {
         for (i = 0; i < cdpHost->nChars; i++) {
            s_charTrans[(BYTE)cdpHost->CharsUpper[i]] = (BYTE)cdpTerm->CharsUpper[i];
            s_charTrans[(BYTE)cdpHost->CharsLower[i]] = (BYTE)cdpTerm->CharsLower[i];
         }
      }
   }
#else
...
void HB_GT_FUNC(gt_SetKeyCP(char *pszTermCDP, char *pszHostCDP)) {
   ...
   if (pszTermCDP && pszHostCDP && *pszTermCDP && *pszHostCDP) {
      ...
      if (cdpTerm && cdpHost && cdpTerm != cdpHost && cdpTerm->nChars && cdpTerm->nChars == cdpHost->nChars) {
         for (i = 0; i < cdpHost->nChars; i++) {
            s_keyTrans[(BYTE)cdpHost->CharsUpper[i]] = (BYTE)cdpTerm->CharsUpper[i];
            s_keyTrans[(BYTE)cdpHost->CharsLower[i]] = (BYTE)cdpTerm->CharsLower[i];
         }
      }
   }
#else
